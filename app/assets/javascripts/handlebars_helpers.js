...
Handlebars.registerHelper('save_or_delete_from_profile_mini', function() {
  var couponID = this.id;
  var foundUserCoupon = userObject.coupons.find( function (coupon) { return coupon.id === couponID})
  if (foundUserCoupon){
    return new Handlebars.SafeString(`<button type="button" class="btn btn-sm btn-outline-secondary" data-couponId="${couponID}" id="delete-from-profile">Delete from profile</button>`)
  } else {
    return new Handlebars.SafeString(`<button type="button" class="btn btn-sm btn-outline-secondary" data-couponId="${couponID}" id="save-to-profile">Save to profile</button>`)
  }
})